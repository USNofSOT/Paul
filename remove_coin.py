...
async def removecoin(ctx, target: discord.Member):
    if check_permissions(ctx.author, ["Board of Admiralty"]):
        try:
			...
			try:
				...
				if content.isdigit():
					...
					if 0 <= coin_index < len(coins):
						...
						database_manager.remove_coin(coins[coin_index][0])  # Assuming coin[0] is the coin ID

						embedVar = discord.Embed(title="Challenge Coins", color=0x00ff00, 
												description=f"Removed challenge coin from the database.")
					...
					await ctx.send("Invalid input. Please respond with a valid number to remove a coin.")
            ...
            print(f"Error removing coin: {e}")
            await ctx.respond("An error occurred while removing the coin.", ephemeral=True)
            return
	else:  
