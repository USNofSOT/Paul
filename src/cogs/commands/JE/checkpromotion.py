    ...
    async def view_moderation(self, interaction: discord.interactions, target: Union[discord.Member, discord.Role] = None):
        ...
        if isinstance(target, discord.Member):
            ...
            if current_rank is None:
                ...
                await interaction.response.followup.send(embed=embed, ephemeral=False)
                return
        ...
        if isinstance(target, discord.Role):
            ...
            for member in members:
                ...
                mass_audit_log_repository = AuditLogRepository()
                mass_voyage_repository = VoyageRepository()

                mass_guild_member = self.bot.get_guild(GUILD_ID).get_member(member.id)
                mass_guild_member_role_ids = [role.id for role in mass_guild_member.roles]
                mass_netc_guild_member = self.bot.get_guild(NETC_GUILD_ID).get_member(member.id)
                if mass_netc_guild_member:
                    netc_guild_member_role_ids = [role.id for role in mass_netc_guild_member.roles]
                else:
                    netc_guild_member_role_ids = []
                mass_is_marine = MARINE_ROLE in mass_guild_member_role_ids

                mass_sailor_repository = SailorRepository()
                mass_sailor: Sailor = mass_sailor_repository.get_sailor(member.id)
                mass_sailor_repository.close_session()

                mass_voyage_count: int = mass_sailor.voyage_count + mass_sailor.force_voyage_count or 0
                mass_hosted_count: int = mass_sailor.hosted_count + mass_sailor.force_hosted_count or 0
                mass_embed = default_embed(
                    title=f"{member.display_name or member.name}",
                    description=f"{member.mention}",
                    author=False
                )
                try:
                    avatar_url = mass_guild_member.guild_avatar.url if mass_guild_member.guild_avatar else mass_guild_member.avatar.url
                    mass_embed.set_thumbnail(url=avatar_url)
                except AttributeError:
                    pass
                mass_current_rank: NavyRank = get_current_rank(mass_guild_member)
                if mass_current_rank is None:
                    mass_embed.add_field(
                        name="Current Rank",
                        value="No rank found",
                    )
                    await interaction.response.followup.send(embed=mass_embed, ephemeral=False)
                    return
                mass_current_rank_name = mass_current_rank.name if not mass_is_marine else mass_current_rank.marine_name
                if E2_ROLES[1] in mass_guild_member_role_ids:
                    mass_current_rank_name = "Seaman Apprentice"

                mass_embed.add_field(
                    name="Current Rank",
                    value=f"{mass_current_rank_name}",
                )

            for mass_rank_index in mass_current_rank.promotion_index:
                mass_next_rank = get_rank_by_index(mass_rank_index)

                mass_requirements=""
                mass_additional_requirements=[]
                mass_has_next = False
                match mass_next_rank.index:
                    case 3: # Able Seaman

                        # if user is seaman apprentice
                        if E2_ROLES[1] in mass_guild_member_role_ids:
                            mass_latest_apprentice_role_log = mass_audit_log_repository.get_latest_role_log_for_target_and_role(member.id, E2_ROLES[1])
                            if not mass_latest_apprentice_role_log:
                                mass_days_with_apprentice = 0
                                mass_requirements += f"\u200b \n **:warning: Please verify role age by hand whilst bot is new**  \n \n"
                            else:
                                days_with_apprentice = get_time_difference_in_days(utc_time_now(), latest_apprentice_role_log.log_time) if latest_apprentice_role_log else None
                            if days_with_apprentice >= 14:
