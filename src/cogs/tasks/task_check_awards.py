...
def fake_context(bot, GUILD, channel_name="test-channel"):
    ...
    ctx.guild.name = "USN"
    ctx.guild.roles = None
    ctx.guild.members = None
    ctx.guild.get_member = None
    ctx.send = AsyncMock()
    return ctx
...
def create_award_messages(role, sailor_repo, GUILD, context, exclude_roles=[]):
    ...
    for member in role.members:
        ...
        for ex_role in exclude_roles:
            if hasattr(sailor, "roles") and ex_role in sailor.roles:
                exclude = True
                break
        if exclude:
            continue
    ...
    async def my_task(self):
        ...
        try:
            ...
            for ship in SHIPS:
                ...
                award_msgs = create_award_messages(
                    role, sailor_repo, GUILD, context, exclude_roles=[boa_role]
                )
                for msg_str in award_msgs:
                    await channel.send(msg_str)

            # BOA members
            log.info("Checking awards for BOA")
