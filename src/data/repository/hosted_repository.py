...
class HostedRepository:
    ...
    def get_hosted_by_either_ship_name_or_auxiliary_ship_name(
        self, ship_name: str = None, auxiliary_ship_name: str = None, amount: int = None
    ) -> list[Type[Hosted]]:
        try:
            query = self.session.query(Hosted).order_by(Hosted.log_time.desc())
            if ship_name and auxiliary_ship_name:
                query = query.filter(
                    Hosted.ship_name == ship_name,
                    Hosted.auxiliary_ship_name == auxiliary_ship_name,
                )
            elif ship_name:
                query = query.filter(
                    Hosted.ship_name == ship_name, Hosted.auxiliary_ship_name is None
                )
            elif auxiliary_ship_name:
                query = query.filter(Hosted.auxiliary_ship_name == auxiliary_ship_name)
            return query.limit(amount).all()
        except Exception as e:
            log.error("Error getting hosted log entries by ship name or auxiliary ship name")
            raise e
    ...
    def get_hosted_by_role_ids_and_between_dates(
        ...
        try:
            ...
            log.error("Error getting voyage log entries by role IDs and between dates")
            raise e
    ...
    def get_hosted_by_role_ids_and_target_ids_and_between_dates(
        ...
        try:
            ...
            log.error("Error getting voyage log entries by role IDs, target IDs, and between dates")
            raise e
    ...
    def get_host_by_log_id(self, log_id: int) -> Hosted | None:
        try:
            ...
            log.error("Error getting hosted log entry by log ID")
            raise e
    ...
    def get_all_main_ship_names(self):
        try:
            return [name for (name,) in self.session.query(Hosted.ship_name).distinct() if name]
        except Exception as e:
            log.error("Error getting all main ships")
            raise e

    def get_all_auxiliary_ship_names(self):
        try:
            return [
                name
                for (name,) in self.session.query(Hosted.auxiliary_ship_name).distinct()
                if name
            ]
        except Exception as e:
            log.error("Error getting all auxiliary ships")
            raise e

    def get_all_auxiliary_ship_names_by_main_ship(self, main_ship):
        try:
            return [
                name
                for (name,) in self.session.query(Hosted.auxiliary_ship_name)
                .filter(Hosted.ship_name == main_ship)
                .distinct()
                if name
            ]
        except Exception as e:
